...
from r2_gaussian.utils.fast_utils import sampling_cameras, compute_gaussian_score_r2gs
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                if (
                    ...
                    # VCD：多视角一致性致密化
                    # 采样 6 个视角，计算 importance_score 和 pruning_score
                    vcd_camlist = sampling_cameras(
                        scene.getTrainCameras(), num_cams=6
                    )
                    importance_score, pruning_score = compute_gaussian_score_r2gs(
                        vcd_camlist, gaussians, pipe,
                        DENSIFY=True,
                        quantile=0.70,
                    )
            ...
            # VCP：最终阶段多视角剪枝（15k 之后每 3k 轮一次）
            if (
                iteration >= opt.densify_until_iter
                and iteration % 3000 == 0
                and iteration <= opt.iterations
            ):
                vcp_camlist = sampling_cameras(
                    scene.getTrainCameras(), num_cams=30
                )
                _, pruning_score = compute_gaussian_score_r2gs(
                    vcp_camlist, gaussians, pipe,
                    DENSIFY=False,
                    quantile=0.70,
                )
                gaussians.densify_and_prune(
                    ...
                    pruning_score=pruning_score,
                    prune_score_thresh=0.95,   # VCP 阶段更保守
                )
