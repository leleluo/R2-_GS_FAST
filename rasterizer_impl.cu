...
int CudaRasterizer::Rasterizer::forward(
	...
	const int mode,
	const float* pixel_error_map,        // ★ 新增（device ptr）
    float* gaussian_responsibility,      // ★ 新增（device ptr）
    float* gaussian_err_count,   // ★ 新增
	float* out_color,
	int* radii,
	bool debug)
{	
    ...
    // ★ 每次 forward 前清零输出（否则累积污染）
    if (gaussian_responsibility != nullptr) {
        CHECK_CUDA(cudaMemset(gaussian_responsibility, 0, P * sizeof(float)), debug);
    }
    if (gaussian_err_count != nullptr) {
        CHECK_CUDA(cudaMemset(gaussian_err_count, 0, P * sizeof(float)), debug);
    }
	...
	CHECK_CUDA(FORWARD::render(
		...
		geomState.mus,
		imgState.n_contrib,
		pixel_error_map,          // ★ 从 Python 传下来的
        gaussian_responsibility,  // ★ CUDA 输出
