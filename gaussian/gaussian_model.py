...
class GaussianModel:
    ...
    def densify_and_split(
        ...
        # ★ 新增：importance_score 过滤（对新高斯补 0，保证长度一致）
        if importance_score is not None:
            if importance_score.shape[0] < selected_pts_mask.shape[0]:
                pad = torch.zeros(
                    selected_pts_mask.shape[0] - importance_score.shape[0],
                    device=importance_score.device,
                )
                importance_score = torch.cat([importance_score, pad], dim=0)

            importance_mask = importance_score > importance_thresh
            selected_pts_mask = torch.logical_and(selected_pts_mask, importance_mask)
    ...
    def densify_and_clone(
        ...
        # ★ 新增：importance_score 过滤（对新高斯补 0）
        if importance_score is not None:
            if importance_score.shape[0] < selected_pts_mask.shape[0]:
                pad = torch.zeros(
                    selected_pts_mask.shape[0] - importance_score.shape[0],
    ...
    def densify_and_prune(
        ...
        grads_raw[grads_raw.isnan()] = 0.0

        n = self.get_xyz.shape[0]
        m = grads_raw.shape[0]

        if m < n:
            pad = torch.zeros((n - m, 1), device=grads_raw.device)
            grads = torch.cat([grads_raw, pad], dim=0)
        else:
            grads = grads_raw
        # --------------------------------------
        ...
        if pruning_score is not None:
            if pruning_score.shape[0] < prune_mask.shape[0]:
                pad = torch.zeros(
                    prune_mask.shape[0] - pruning_score.shape[0],
                    device=pruning_score.device
                )
