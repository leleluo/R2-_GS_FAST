...
renderCUDA(
	...
	const float* __restrict__ pixel_error_map,   // ★ 新增
    float* __restrict__ gaussian_responsibility, // ★ 新增
    float* __restrict__ gaussian_err_count,           // ★ 输出：每高斯高误差像素计数（可为 nullptr）
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < CHANNELS; ch++)
				C[ch] += alpha;
            // ===== FastGS / R2-GS 统计（新增）=====
            if (pixel_error_map != nullptr) {
                float m = pixel_error_map[pix_id];  // 0/1 mask（或连续误差也行，但count建议用0/1）
                if (m > 0.5f) {
                    // 1) count：该高斯覆盖到了一个“高误差像素”
                    if (gaussian_err_count != nullptr) {
                        atomicAdd(&gaussian_err_count[collected_id[j]], 1.0f);
                        // 如果你想 alpha 加权计数： atomicAdd(&gaussian_err_count[collected_id[j]], alpha);
                    }

                    // 2) responsibility：可选，保留你原来的“误差责任”
                    if (gaussian_responsibility != nullptr) {
                        atomicAdd(&gaussian_responsibility[collected_id[j]], alpha);
                        // 若你坚持用 alpha*err（连续误差图）：atomicAdd(&gaussian_responsibility[collected_id[j]], alpha * m);
                    }
                }
            }
...
void FORWARD::render(
	...
	const float* mus,
	uint32_t* n_contrib,
	const float* pixel_error_map,        // ★ 新增
    float* gaussian_responsibility,      // ★ 新增
    float* gaussian_err_count,
	float* out_color)
{
